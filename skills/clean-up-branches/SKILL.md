...
5. Show the exact local and remote deletion sets before mutating them. A direct
   request with scope meeting the 95% confidence threshold authorizes deletion of
   only the verified set; if the skill was selected without an explicit cleanup
   request, ask for approval first. Never remove a worktree or delete an uncertain
   branch.
